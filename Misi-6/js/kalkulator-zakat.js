document.addEventListener("DOMContentLoaded", () => {
  ...
  const inputAsetLancarPerdagangan = document.getElementById(
    "aset-lancar-perdagangan",
  );
  ...
  const zakatAmountContainer = document.getElementById(
    "zakat-amount-container",
  );
  ...
  inputAsetLancarPerdagangan.addEventListener(
    "input",
    calculateJumlahPerdagangan,
  );
  ...
  zakatTypeSelect.addEventListener("change", () => {
    ...
    if (type === "penghasilan") {
      ...
      emptyDesc.textContent =
        "Zakat penghasilan wajib ditunaikan jika penghasilan bulanan Anda (gaji + lain-lain) mencapai nisab sebesar 85 gram emas per bulan. Lengkapi form di sebelah kiri untuk melihat kewajiban zakat Anda (2,5%).";
    } else {
      if (type === "perusahaan") {
        formPerusahaan.classList.remove("hidden");
        labelTotal.textContent = "Total Aset Bersih Perusahaan:";
        emptyDesc.textContent =
          "Zakat perusahaan dihitung dari aset bersih (total aktiva lancar dikurangi utang jangka pendek) yang telah berumur satu tahun (haul). Nisabnya setara 85 gram emas murni (2,5%).";
      } else if (type === "perdagangan") {
        formPerdagangan.classList.remove("hidden");
        labelTotal.textContent = "Total Harta Perdagangan Bersih:";
        emptyDesc.textContent =
          "Zakat perdagangan dikenakan pada modal yang diputar, keuntungan, dan piutang lancar, dikurangi utang jatuh tempo. Jika melebihi nisab 85 gram emas per tahun, zakat wajib dikeluarkan (2,5%).";
      } else if (type === "emas") {
        formEmas.classList.remove("hidden");
        labelTotal.textContent = "Total Nilai Rupiah Emas:";
        emptyDesc.textContent =
          "Zakat emas wajib ditunaikan bagi yang memiliki emas lebih dari nisab 85 gram selama setahun. Masukkan nilai rupiah emas Anda saat ini. Tarif zakat adalah 2,5%.";
      }
  ...
  resetBtn.addEventListener("click", () => {
    ...
    if (window.innerWidth <= 900) {
      document
        .querySelector(".calculator-form-container")
  ...
  calculateBtn.addEventListener("click", () => {
    ...
    if (type === "penghasilan") {
      ...
      total = gaji + lain;
      nisab = NISAB_BULAN;
      labelZakatAmount.textContent =
        "Jumlah Zakat yang Harus Ditunaikan per Bulan:";
      ...
      if (type === "perusahaan") {
        const activeTab = document
          .querySelector(".baznas-tab.active")
    ...
    if (window.innerWidth <= 900 && total >= nisab) {
      document
        .querySelector(".calculator-result-container")
        .scrollIntoView({ behavior: "smooth" });
    }
  });
