document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("global-date").textContent =
    new Date().toLocaleDateString("id-ID", dateOpts);
  ...
  function isShalatTimeReached(shalatName) {
    if (!appData.shalatSchedule || !appData.shalatSchedule[shalatName])
      return true;
  ...
  async function fetchShalatSchedule() {
    ...
    if (appData.shalatScheduleDate === todayStr && appData.shalatSchedule) {
      updateShalatTimeDOM();
      if (locTextEl)
        locTextEl.textContent = appData.shalatLocationName || "📍 Jakarta";
      return;
    }
    ...
    if ("geolocation" in navigator) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          console.error(
            "Geolocation denied/failed. Falling back to Jakarta.",
            error
          );
          fallbackFetch();
        },
  ...
  function updateShalatStats() {
    ...
    document.getElementById("shalat-progress-fill").style.width =
      `${percentage}%`;
    ...
    document.getElementById("shalat-status-text").textContent =
      `${percentage}% - ${status}`;
  }
  ...
  function initQuranUI() {
    ...
    if (qBtnEditTarget) {
      qBtnEditTarget.addEventListener("click", () => {
        if (!appData.quran.targetLastUpdate) {
          ...
          qQuranTarget.focus();
          showToast(
            "Target tilawah harian baru diset. Silakan tentukan targetmu!",
            "success",
          );
        ...
        if (diffDays >= 3) {
          ...
          showToast(
            "Konsistensi terjaga! Anda dapat memperbarui target tilawah hari ini.",
            "success",
          );
        } else {
          const daysLeft = 3 - diffDays;
          showToast(
            `Target membaca Qur'an terkunci demi konsistensi. Bisa diubah ${daysLeft} hari lagi.`,
            "warning",
          );
        }
      });
    ...
    qQuranTarget.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        qQuranTarget.disabled = true;
        showToast(
          "Target tilawah harian telah disimpan dan dikunci.",
          "success",
        );
      }
    });
  ...
  function updateQuranStats() {
    ...
    document.getElementById("quran-progress-fill").style.width =
      `${visualPercentage}%`;
    ...
    document.getElementById("quran-status-text").textContent =
      `${percentage}% - ${status}`;
  }
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      if (i <= todayDay) {
        div.addEventListener("click", () => {
          if (status) {
            showToast(
              "Data hari ini sudah terkunci dan tidak bisa diubah.",
              "error",
            );
            return;
          }
  ...
  const puasaStatusButtons = document.querySelectorAll(
    ".btn-status-puasa, .btn-status-halangan, .btn-status-tidak",
  );
  ...
  function initGenderLogic() {
    ...
    document
      .getElementById("gender-female-btn")
  ...
  puasaStatusButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (status === "puasa") {
        ...
        appData.puasa.checkedDays = appData.puasa.checkedDays.filter(
          (d) => d !== activeEditingDay,
        );
      }
      ...
      if (status === "halangan" || status === "tidak-puasa") {
        ...
        if (status === "halangan") {
          ...
          advice =
            "Jangan lupa ganti (Qadha) puasa ya nanti, dan pastikan tetap menjaga kesehatan selama Ramadhan! ✨";
        }
  ...
  function updatePuasaStats() {
    ...
    let todayDay = getCurrentRamadanDay();
    document.getElementById("puasa-day-indicator").textContent =
      `Hari ke-${todayDay}`;
    document.getElementById("puasa-progress-fill").style.width =
      `${percentage}%`;
    document.getElementById("puasa-status-text").textContent =
      `${checked}/30 Hari Puasa - Semangat!`;
  ...
  function handleDzikirChange() {
    ...
    dzikirCount =
      parseInt(localStorage.getItem(`misi4_dzikir_count_${currentDzikirId}`)) ||
      0;
  ...
  function initDzikirUI() {
    ...
    if (savedDate !== today) {
      ...
      dzikirTotalDailyCount =
        parseInt(localStorage.getItem("misi4_dzikir_total_daily")) || 0;
    }
  ...
  function showToast(message, type = "success") {
    ...
    let icon =
      '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#03522f" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>';
    if (type === "error")
      icon =
